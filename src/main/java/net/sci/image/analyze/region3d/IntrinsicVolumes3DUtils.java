...
public class IntrinsicVolumes3DUtils
{
    ...
    private final static double[] surfaceAreaLutD3(Calibration calib) 
    {
        ...
        double d1 = calib.getXAxis().getSpacing();
        double d2 = calib.getYAxis().getSpacing();
        double d3 = calib.getZAxis().getSpacing();
        double vol = d1 * d2 * d3;
    ...
    private static final double[] meanBreadthLutD3(Calibration calib, int conn2d)
    {
        ...
        // correspondence map between voxel label and voxel coords within config
    ...
    public static final double[] directionWeights3d13(Calibration calib) 
    {
        ...
        double dx = calib.getXAxis().getSpacing();
        double dy = calib.getYAxis().getSpacing();
        double dz = calib.getZAxis().getSpacing();
	 ...
	 *         voxels
	 */
    public static final double samplingVolume(Array3D<?> image, Calibration calib)
    {
        // size of image
        int sizeX = image.size(0);
        int sizeY = image.size(1);
        int sizeZ = image.size(2);
        return (sizeX - 1) * calib.getXAxis().getSpacing() * (sizeY - 1) * calib.getYAxis().getSpacing() * (sizeZ - 1) * calib.getZAxis().getSpacing();   
    }

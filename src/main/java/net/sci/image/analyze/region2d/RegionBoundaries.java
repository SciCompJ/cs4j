...
public class RegionBoundaries
{
	 ...
	 */
	public final static Map<Integer, ArrayList<Point2D>> runLengthsCornersMap(IntArray2D<?> image, int[] labels)
	{
	 ...
	 */
	public final static ArrayList<Point2D>[] runlengthsCorners(IntArray2D<?> image, int[] labels)
	{
	 ...
	 * @return a list of points that can be used for convex hull computation
	 */
	public final static ArrayList<Point2D> runLengthsBoundaryPixels(IntArray2D<?> image)
	{
		// size of input image
		int sizeX = image.size(0);
		int sizeY = image.size(1);
		...
		for (int y = 0; y < sizeY; y++)
		{
			...
			for (int x = 0; x < sizeX; x++)
			{
				if (image.getInt(x, y) > 0 && !inside)
				{
					// transition from background to foreground
					points.add(new Point2D(x, y));
					inside = true;
				} 
				else if (image.getInt(x, y) == 0 && inside)
				{
	 ...
	 * @return a list of points that can be used for convex hull computation
	 */
	public final static ArrayList<Point2D> runLengthsCorners(IntArray2D<?> image)
	{
		int sizeX = image.size(0);
		int sizeY = image.size(1);
	...
	public static final ArrayList<Point2D> boundaryPixelsMiddleEdges(IntArray2D<?> binaryImage)
	{
		// size of image
		...
		for (int y = 0; y < sizeY + 1; y++) 
		{
			...
			for (int x = 0; x < sizeX + 1; x++) 
			{
				...
				configValues[1] = x < sizeX & y > 0 ? binaryImage.getInt(x, y - 1) > 0: false;
				configValues[3] = x < sizeX & y < sizeY ? binaryImage.getInt(x, y) > 0: false;

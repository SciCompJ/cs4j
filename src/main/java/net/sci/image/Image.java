...
public class Image
{
    ...
    {
        this.data = data;
        this.type = ImageType.chooseBestType(data);
        computeImageSize();
        initCalibration();
        setupDisplayRange();
    }
    ...
    private void copyDisplaySettings(Image parent)
    {
        ...
        // (escape the case of binary images, that should not be considered as
        // intensity nor distance)
        if (parent.type.isCompatibleWith(this.data) && this.data.elementClass() != Binary.class)
        {
            ...
            {
                // Additional processing to propagate settings only if same data
                // type
    ...
    /**
     * @return the name of this image together with the extension, if it is not
     *         null.
     */
    public String getFullName()
    {
        if (this.extension.isEmpty()) return this.name;
    ...
    public void setNameFromFileName(String fileName)
    {
        ...
        {
            // special case of a file name starting with a dot -> keep the full
            // name.
            this.name = fileName;
        }
    }
    ...
    private static final String[] splitFileName(String fileName)
    {
        ...
        else
        {
            ...
            return new String[] { baseName.substring(0, extIndex), baseName.substring(extIndex + 1) };
        }
    }
    ...
    {
        return isGrayscaleImage() || isLabelImage() || this.type == ImageType.INTENSITY
                || this.type == ImageType.DISTANCE || this.type == ImageType.DIVERGING;
    }
    ...
    {
        return this.type == ImageType.COLOR || this.type == ImageType.VECTOR || this.type == ImageType.COMPLEX
                || this.type == ImageType.GRADIENT;
    }

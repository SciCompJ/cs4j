...
public class OtsuThreshold extends AutoThreshold
{
     ...
     * @see net.sci.image.process.segment.AutoThreshold#computeThresholdValue(net.sci.array.data.ScalarArray)
     */
    public double computeThresholdValue(ScalarArray<?> array, BinaryArray mask)
    {
        double minValue = Double.POSITIVE_INFINITY;
        double maxValue = Double.NEGATIVE_INFINITY;
        for (double v : mask.selectValues(array))
        {
            minValue = Math.min(v,  minValue);
            maxValue = Math.max(v,  maxValue);
        }
        
        // bounds for computing histogram
    ...
    public double computeThresholdValue(ScalarArray<?> array, BinaryArray mask, double[] range, int nLevels)
    {
        // compute the array of possible thresholds
        double[] levels = Histograms.computeBinPositions(range, nLevels);
        
        // Compute count histogram
        int[] histo = Histograms.histogramScalar(mask.selectValues(array), range, nLevels);

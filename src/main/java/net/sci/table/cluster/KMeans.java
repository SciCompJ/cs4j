...
public class KMeans implements TableOperator
{
    ...
    public Table centroids()
    {
        ...
        // create table
        Axis rowAxis = new CategoricalAxis("Class", rowNames);
        NumericTable res = NumericTable.create(rowAxis, nf);
    ...
    public KMeans fit(Table table)
    {
        ...
        if (nr < np)
        {
            throw new IllegalArgumentException(
                    "Requires a table with at least as many rows as columns");
        }
    ...
    public Table predict(Table table)
    {
        ...
        if (nc != this.centroids[0].length)
        {
            throw new IllegalArgumentException(String.format(
                    "Number of columns of table (%d) does not match centroids dimension (%d)", nc,
